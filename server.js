// server.js — robust Goodreads scraper + CORS + timeouts + auto-relaunch

import express from 'express';
import { setTimeout as delay } from 'timers/promises';

// Use Playwright browsers baked into Docker image (Render)
...
const corsMiddleware = (req, res, next) => {
  // For simplicity allow any origin. If you want to restrict, replace "*" with "http://localhost:8000"
...
async function getBrowser({ timeoutMs = 20000 } = {}) {
  ...
  return await withDeadline(launching, timeoutMs, 'Chromium launch');
}
...
async function runScrape(isbn) {
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      // Navigate to first book result (reliable selector)
      ...
      const [
        ...
        getText(page.locator('dt:has-text("Format") + dd'), 5500),
      ...
      // Parse format/pageCount from the Format row (e.g., “Paperback, 352 pages”)
      let pageCount = null;
      let format = null;
      if (formatText) {
